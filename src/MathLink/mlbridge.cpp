...
void MLBridge::Connect(int newArgc, const char *newArgv[]){
    ...
    link = MLOpen(argc, (char **)argv);
    if (link == NULL) {
        //The link failed to open.
        connected = false;
        MLDeinitialize(environment);
...
std::string MLBridge::GetUTF8String(GetFunctionType func){
    ...
    switch(func){
        case GetString:
            success = MLGetUTF8String(link, &stringBuffer, &bytes, &characters);
            break;
        ...
        case GetSymbol:
            success = MLGetUTF8Symbol(link, &stringBuffer, &bytes, &characters);
            break;
    ...
    MLReleaseUTF8String(link, stringBuffer, bytes);
...
void MLBridge::ErrorCheck(){
    ...
    if(!link){
        throw MLBridgeException("The MathLink connection has been severed.", MLEDEAD);
    }
    errorCode = MLError(link);
    if(errorCode!=0){
        error = "MathLink Error: " + std::string(MLErrorMessage(link));
        Disconnect();
...
void MLBridge::Evaluate(std::string inputString){
    ...
    MLPutUTF8String(link, (const unsigned char *)inputString.data(), (int)inputString.size());
    MLEndPacket(link);
...
bool MLBridge::ReceivedMenuPacket(){
    ...
    MLGetInteger(link, &interruptMenuNumber);
...
bool MLBridge::ReceivedSuspendPacket(){
    ...
    MLNewPacket(link); //Do I need this line?
